...
<div class="card mt-3 p-3">
  ...
  <div id="logs-output" role="tabpanel" aria-labelledby="tab-{tab}" tabindex="0" class="mt-2 max-h-[60dvh] overflow-auto rounded-lg bg-console">
    <pre class="p-3 font-mono text-xs leading-5 text-on-console {wrap ? 'break-words whitespace-pre-wrap' : 'whitespace-pre'}">{text || (loading ? t('app.loading') : t('logs.empty'))}</pre>
  </div>
</div>
