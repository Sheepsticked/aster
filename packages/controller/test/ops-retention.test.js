// @ts-check
// Tests for src/ops/retention.js: only finished operations and notifications past their retention days are deleted, and
// startRetention's schedule.
...
describe('ops retention', () => {
  test('purge deletes finished operations and notifications older than their retention days and keeps everything else', () => {
    ...
    assert.deepEqual(purge(db, { days: { operations: 10, notifications: 5 }, now: NOW }), { operations: 3, notifications: 2 });
    ...
    assert.deepEqual(purge(db, { days: { operations: 10, notifications: 5 }, now: NOW }), { operations: 0, notifications: 0 });
  });
  ...
  test('invalid retention days throw a RangeError before anything is deleted', () => {
    ...
    const cases = [
      [{ operations: 0, notifications: 90 }, 'retention_days.operations must be a whole number of days from 1 to 36500, not 0'],
      [{ operations: 90, notifications: 1.5 }, 'retention_days.notifications must be a whole number of days from 1 to 36500, not 1.5'],
      [{ operations: '90', notifications: 90 }, 'retention_days.operations must be a whole number of days from 1 to 36500, not 90'],
      [{ operations: 90, notifications: 36501 }, 'retention_days.notifications must be a whole number of days from 1 to 36500, not 36501'],
      [{ operations: 90 }, 'retention_days.notifications must be a whole number of days from 1 to 36500, not undefined'],
  ...
  test('startRetention purges after its delay and then at every interval with the days of that moment; a failing run is logged; stop() ends it', (t) => {
    ...
    let clock = NOW;
    /** @type {{ operations: number, notifications: number } | null} */
    let days = { operations: 10, notifications: 10 };
    ...
    assert.deepEqual(remaining(db, 'notifications'), []);
    assert.deepEqual(lines, [['info', 'retention deleted old rows', { operations: 1, notifications: 1 }]]);

    days = { operations: 1, notifications: 10 };
    clock += DAY_MS;
    ...
    days = { operations: 1, notifications: 1 };
    t.mock.timers.tick(DAY_MS);
    assert.deepEqual(remaining(db, 'operations'), []);
    t.mock.timers.tick(DAY_MS);
